    ...
    function renderDetails(detailObj) {
        ...
        if (detailObj.song_list && detailObj.song_list.length > 0) {
            ...
            detailObj.song_list.forEach(song => {
                ...
                const title = isObj ? song.title : song;
                const url = isObj ? song.youtube_url : null;

                if (url) {
                    const a = document.createElement("a");
                    a.className = "icon-link icon-link-hover link-underline link-underline-opacity-25 text-danger";
                    a.href = url;
                    a.target = "_blank";

                    const icon = document.createElement("i");
                    icon.className = "bi bi-youtube";  // ← just the class, no inner path needed
